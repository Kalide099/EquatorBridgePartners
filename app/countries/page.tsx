        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl space-y-8">
            <h1 className="text-5xl md:text-7xl font-black leading-tight">
              {t("countries_page.hero_title")} <span className="text-secondary-400 font-extrabold uppercase tracking-tight">{t("countries_page.hero_title_highlight")}</span>
            </h1>
            <p className="text-xl md:text-2xl text-slate-400 leading-relaxed font-semibold">
               {t("countries_page.hero_subtitle")}
            </p>
          </div>
        </div>
      </section>
              ...
              {countries.map((country, idx) => (
                <div key={idx} className="group relative bg-white rounded-[2rem] border-2 border-slate-100 p-10 transition-all duration-300 hover:shadow-2xl hover:border-primary-600/20 transform hover:-translate-y-2">
                   ...
                   <div className="space-y-4 mb-10 pt-6 border-t border-slate-50">
                      <p className="text-slate-400 font-extrabold uppercase tracking-widest text-xs">{t("countries_section.services_available")}</p>
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-slate-900 rounded-[3rem] p-12 md:p-24 text-white flex flex-col md:flex-row items-center gap-16 shadow-2xl overflow-hidden relative">
               ...
               <div className="flex-1 space-y-8 relative z-10">
                  <h2 className="text-4xl md:text-5xl font-black leading-tight">{t("countries_page.network_title")}</h2>
                  <p className="text-xl text-slate-400 font-medium leading-relaxed">
                     {t("countries_page.network_subtitle")}
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8">
                     <div className="flex items-center space-x-4">
                        <CheckCircle2 className="w-8 h-8 text-secondary-500" />
                        <span className="text-lg font-bold">{t("countries_page.network_feature_1")}</span>
                     </div>
                     <div className="flex items-center space-x-4">
                        <CheckCircle2 className="w-8 h-8 text-primary-500" />
                        <span className="text-lg font-bold">{t("countries_page.network_feature_2")}</span>
                     </div>
                  </div>
               </div>
               <div className="flex-1 relative z-10">
                  <div className="bg-white/5 backdrop-blur-md rounded-3xl p-12 border border-white/10 text-center">
                     <p className="text-primary-400 font-black uppercase tracking-widest text-sm mb-4">{t("countries_page.stat_badge")}</p>
                     <p className="text-7xl font-black mb-6">9+</p>
                     <p className="text-xl font-bold text-slate-400 uppercase tracking-widest leading-relaxed">{t("countries_page.stat_subtitle")}</p>
                  </div>
               </div>
            </div>
         </div>
